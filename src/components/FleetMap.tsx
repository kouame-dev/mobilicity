...
export default function FleetMap({
  ...
  const [showTerrain, setShowTerrain] = useState<'streets' | 'satellite' | 'relief' | 'radar'>('streets');
  ...
  return (
    <div id="fleet-map-container" className="relative bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden h-[540px] shadow-2xl flex flex-col">
      ...
      <div id="map-control-bar" className="absolute top-4 left-4 right-4 z-10 flex flex-wrap gap-2 items-center justify-between pointer-events-none">
        <div className="flex flex-wrap gap-1.5 pointer-events-auto bg-slate-900/95 backdrop-blur-md p-1 rounded-xl border border-slate-800 shadow-md">
          <button
          ...
          <button
            onClick={() => setShowTerrain('satellite')}
            className={`px-2.5 py-1 text-[11px] font-semibold rounded-lg transition-all ${
              showTerrain === 'satellite' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
            }`}
          >
            🛰️ Satellite
          </button>
          <button
            onClick={() => setShowTerrain('relief')}
            className={`px-2.5 py-1 text-[11px] font-semibold rounded-lg transition-all ${
              showTerrain === 'relief' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white'
            }`}
          >
            ⛰️ Relief
          </button>
        ...
        {showTerrain !== 'radar' ? (
          <InteractiveMap
            markers={vehicles.map(v => ({
              ...
              isLocked: v.isLocked,
              batteryPercent: Math.round((v.powerAvailableWh / v.batteryCapacityWh) * 100)
            }))}
            selectedMarkerId={selectedVehicleId}
            onSelectMarker={onSelectVehicle}
            mapType={showTerrain}
          />
        ) : (
          <>
            {/* Background Grid Lines representing street maps */}
            <div className="absolute inset-0 transition-all duration-500 bg-slate-950 bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:24px_24px]">
